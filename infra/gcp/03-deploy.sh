#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Deploy the 4 HTTP services + the ARQ worker pool to Cloud Run.
...
# ── closet-service (public surface; protected by shared JWT + internal token) ─
echo "▶ deploy $SVC_CLOSET"
gcloud run deploy "$SVC_CLOSET" --image "$IMAGE_BASE/closet-service:$TAG" "${COMMON[@]}" \
  --port 8003 --allow-unauthenticated --min-instances 0 --max-instances 5 \
  --memory 1Gi --cpu 1 --timeout 300 --concurrency 80 \
  --set-env-vars "${D}ENVIRONMENT=production##DEBUG=false##LOG_LEVEL=INFO##REDIS_CHECK_ON_READY=true##${OPENAI_COMMON}##VISION_PREVIEW_FAST=true##RAG_VECTOR_STORE=pgvector##RUN_MIGRATIONS_ON_STARTUP=true##FAIL_ON_STARTUP_MIGRATION_ERROR=true##ALLOWED_ORIGINS=${ALLOWED_ORIGINS}##GATEWAY_INTERNAL_URL=${URL_API}##AI_AGENT_URL=${URL_AGENT}##GCS_BUCKET_NAME=${GCS_BUCKET_NAME}##GCS_PROJECT_ID=${PROJECT_ID}" \
  --set-secrets "DATABASE_URL=db-url-closet:latest,REDIS_URL=redis-cache-url:latest,REDIS_STATE_URL=redis-state-url:latest,ARQ_REDIS_URL=redis-state-url:latest,JWT_SECRET=jwt-secret:latest,INTERNAL_SERVICE_TOKEN=internal-service-token:latest,OPENAI_API_KEY=openai-api-key:latest,GEMINI_API_KEY=gemini-api-key:latest,SENTRY_DSN=sentry-dsn:latest"
