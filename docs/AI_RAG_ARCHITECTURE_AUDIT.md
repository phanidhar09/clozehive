...
- **[FIXED 2026-07-06] SSRF residual: DNS-rebinding / TOCTOU in `product_url.py`.** The guard was otherwise strong (scheme allowlist; private/loopback/reserved-IP block; per-redirect-hop re-validation; attacker-controlled `og:image` also validated; EXIF stripped; size-capped). But `_validate_url` resolved the host to check the IP, then httpx **re-resolved independently** at connect time — an attacker controlling their own DNS could return a public IP for validation and `169.254.169.254` (cloud metadata) for the fetch. **Remediated** by pinning every fetch (including each redirect hop) to the resolved, validated IP: `_pinned_request` rewrites the request URL to the IP while the `Host` header and TLS `sni_hostname` keep the original hostname, so virtual hosts and certificate verification still work and no second DNS lookup ever happens. Verified: 20 product_url tests incl. rebinding/IPv6/port regression cases.
- **[FIXED 2026-07-06] JWT in WebSocket query string.** `ws://host/ws?token=<jwt>` (`ws.py`) leaked tokens via access/proxy logs, browser history, and Referer. **Remediated** with the one-time-ticket exchange: authenticated `POST /api/v1/ws/ticket` issues a 60-second single-use ticket (Redis `GETDEL` redemption, fails closed when the store is down), the client connects with `?ticket=`, and JWT query auth is removed entirely. Frontend `wsClient` fetches a fresh ticket per (re)connect. Verified: 3 ticket tests, frontend typecheck/lint green.
...
> **[FIXED 2026-07-06 — chat path] Structured/constrained extraction of web-intelligence answers.** `trend_grounding.extract_trend_attributes` now schema-validates the (already-sanitized) Tavily answer against curated allowlists (colours / silhouettes / materials / styles), split into trending vs fading by sentence-level cues, and `build_trend_block` builds the system-prompt block **only** from those validated terms plus source *domains* (no free-text titles). Raw web prose never enters the chat prompt; if nothing survives validation the block degrades to "" (pre-Phase-7 behaviour). This raises the injection floor on the chat path rather than just lowering the odds. **Residual (accepted):** festival/venue/dress-guideline consumers are open-vocabulary by nature (proper nouns, rules prose) and still receive chokepoint-sanitized prose — revisit if those surfaces become chat-facing or high-traffic. Verified: 22 trend-grounding tests incl. injection-only-answer → empty block.
