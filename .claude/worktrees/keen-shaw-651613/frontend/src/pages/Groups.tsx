...
import {
  getMyGroups, discoverGroups, createGroup, joinGroup,
  inviteToGroup, removeMember, changeMemberRole, deleteGroup,
  searchUsers
} from '@/lib/api'
...
function CreateGroupModal({ onClose, onCreate }: {
  ...
  const submit = async () => {
    ...
    setError('')
    try {
      const group = await createGroup({ name: name.trim(), description: description.trim() || undefined, is_public: isPublic })
      onCreate(group)
      onClose()
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Failed to create group'
      setError(msg)
    } finally {
      setLoading(false)
    }
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              autoFocus
            />
            <p className="text-xs text-slate-400">Ask a group admin for their 8-character invite code</p>
          </div>
        </div>
        <div className="flex gap-3 p-5 border-t border-cream-200 dark:border-slate-700">
          <button onClick={onClose} className="btn-ghost flex-1">Cancel</button>
...
function GroupDetail({ group, onClose, onUpdate }: {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      searchUsers(userQuery).then(setUsers).catch(() => {})
    }, 400)
  ...
  const invite = async (userId: number) => {
    setInviting(userId)
    try {
      const updated = await inviteToGroup(group.id, userId)
      onUpdate(updated)
    } catch { /* ignore */ }
  ...
  const remove = async (userId: number) => {
    ...
    try {
      ...
      onUpdate({ ...group, members: group.members.filter(m => m.id !== userId), member_count: group.member_count - 1 })
    } catch { /* ignore */ }
  }
  ...
  const toggleRole = async (member: GroupMember) => {
    ...
    try {
      await changeMemberRole(group.id, member.id, newRole)
      onUpdate({ ...group, members: group.members.map(m => m.id === member.id ? { ...m, role: newRole } : m) })
    } catch { /* ignore */ }
  }
            ...
            <div className="space-y-2">
              ...
              {users.filter(u => !group.members.find(m => m.id === u.id)).map(u => (
                <div key={u.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-cream-50 dark:bg-slate-700/50">
                  ...
                  >
                    {inviting === u.id ? <Loader2 size={12} className="animate-spin" /> : 'Invite'}
                  </button>
                </div>
              ))}
            </div>
...
export default function Groups() {
  ...
  // Real-time-ish: poll every 30s
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [mine, pub] = await Promise.all([getMyGroups(), discoverGroups()])
      setMyGroups(mine)
      setPublicGroups(pub)
    } catch (e) {
      console.error(e)
      ...
      <div className="flex items-center gap-3">
        <div className="flex gap-1 bg-cream-100 dark:bg-slate-800 p-1 rounded-xl">
          {[
            ...
            { id: 'discover' as const, label: `Discover (${publicGroups.length})`,  icon: <Compass size={14} /> },
          ].map(t => (
            <button
              key={t.id}
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="card p-5 space-y-4 animate-pulse">
              ...
              <div className="flex gap-2">
                {[...Array(3)].map((_, j) => (
                  <div key={j} className="w-8 h-8 rounded-full bg-cream-200 dark:bg-slate-700" />
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="card p-12 text-center">
          ...
          <h3 className="font-display font-bold text-lg text-slate-800 dark:text-white mb-2">
            {activeTab === 'mine' ? 'No groups yet' : 'No public groups to discover'}
          </h3>
          <p className="text-sm text-slate-400 mb-6">
            {activeTab === 'mine'
              ? 'Create a group to share outfits with friends, family, or colleagues.'
              : 'All public groups are already joined, or none exist yet.'}
          </p>
          {activeTab === 'mine' && (
