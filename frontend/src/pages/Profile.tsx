...
export default function Profile() {
  ...
  const loadSocial = useCallback(async () => {
    ...
    try {
      if (activeTab === 'followers') {
        setFollowers(await socialApi.getFollowers(currentUser.id))
      } else if (activeTab === 'following') {
        setFollowing(await socialApi.getFollowing(currentUser.id))
      } else if (activeTab === 'discover') {
        setDiscover(await socialApi.searchUsers())
      }
    } catch (e) {
      console.error(e)
    } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try { setSearchResults(await socialApi.searchUsers(searchQuery)) }
      catch { /* ignore */ }
  ...
  const saveProfile = async () => {
    setSaveLoading(true)
    try {
      const updated = await authApi.updateProfile({ display_name: editName.trim(), bio: editBio.trim() || undefined })
