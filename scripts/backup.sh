...
BACKUP_FILE="$BACKUP_DIR/closetiq_$TIMESTAMP.sql.gz"
...
if command -v aws &> /dev/null && [ -n "${S3_BACKUP_BUCKET:-}" ]; then
  ...
  echo "Uploaded to S3: s3://$S3_BACKUP_BUCKET/backups/"
fi

find "$BACKUP_DIR" -name "closetiq_*.sql.gz" -mtime +"$RETENTION_DAYS" -delete
