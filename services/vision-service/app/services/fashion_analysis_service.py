...
def _crop_item(image_bytes: bytes, bbox: dict[str, float]) -> bytes:
    """Crop the source image to the given bounding box with a small context margin."""
    try:
...
@traceable(name="vision_fashion_detect_items", run_type="chain")
...
async def _process_single_item(
    ...
    """Crop, remove background, and enrich one detected item. Returns None on skip."""
    ...
    try:
        ...
        final_bytes, _ = await remove_background_async(crop_bytes)
...
@traceable(name="vision_fashion_analyze_image", run_type="chain")
async def analyze_fashion_image(
    ...
    All per-item enrichments run in parallel to minimize latency.
    """
    ...
    results = await asyncio.gather(
        *[_process_single_item(image_bytes, raw, idx) for idx, raw in enumerate(raw_items)],
    ...
    for r in results:
        if isinstance(r, BaseException):
            logger.error("fashion_item_processing_failed", error=str(r))
        elif r is not None:
