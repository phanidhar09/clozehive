...
def _gcs_upload(image_bytes: bytes, blob_name: str, content_type: str) -> str:
    ...
    if cdn:
        # Serve through the CDN edge instead of the origin bucket.
...
def _blob_name_from_gcs_url(url: str) -> str | None:
    ...
    and the CDN form ``{CDN_BASE_URL}/{blob}`` so images uploaded while a CDN was
    configured can still be deleted/read after the fact.
    """
    settings = get_settings()
    cdn = (settings.cdn_base_url or "").strip().rstrip("/")
...
# ── Signed URLs (private bucket reads) ──────────────────────────────────────


@lru_cache(maxsize=1)
def _signing_bucket():
    """Cached GCS bucket handle used purely for local V4 URL signing.

    Signing is an offline crypto operation against the service-account private
    key — no network round-trip — so caching the bucket object is enough.
    """
    settings = get_settings()
...
def _blob_name_from_stored(stored: str) -> str | None:
    ...
    (how uploads are recorded today) or a bare ``uploads/<file>`` blob path.
    """
    blob = _blob_name_from_gcs_url(stored)
    if blob is not None:
        return blob
    # Bare blob path (no scheme, not a local /uploads absolute path).
    if not stored.startswith(("http://", "https://", "/")):
        return stored
    return None


def generate_signed_url(blob_name: str, ttl_seconds: int) -> str:
    """Return a V4 signed GET URL for a blob (offline signing with the SA key)."""
    blob = _signing_bucket().blob(blob_name)
...
def signed_url_for_stored(stored: str | None) -> str | None:
    ...
    (e.g. local-disk dev URLs), or signing fails — so image loading never breaks
    just because the private-bucket path isn't fully configured.
    """
    settings = get_settings()
    if not stored or not settings.gcs_signed_urls or not settings.gcs_enabled:
        return stored
    blob_name = _blob_name_from_stored(stored)
    if not blob_name:
        return stored
    try:
        return generate_signed_url(blob_name, settings.gcs_signed_url_ttl_seconds)
    except Exception as exc:  # noqa: BLE001 — never break reads on a signing hiccup
        logger.warning("signed_url_failed", blob=blob_name, error=str(exc))
        return stored
...
async def _purge_cdn(blob_name: str) -> None:
    """Best-effort CDN edge purge so a deleted image isn't served stale.

    Sends an HTTP PURGE to the object's CDN URL (Fastly/Varnish/nginx-cache).
    If a webhook is configured, also POSTs the URL to it (for Cloudflare-style
    providers that purge via API). All errors are swallowed.
    """
    settings = get_settings()
    ...
    if not cdn:
        return
    cdn_url = f"{cdn}/{blob_name}"
    try:
        import httpx

        async with httpx.AsyncClient(timeout=5.0) as client:
            await client.request("PURGE", cdn_url)
            if settings.cdn_purge_webhook:
                await client.post(settings.cdn_purge_webhook, json={"url": cdn_url})
    except Exception as exc:  # noqa: BLE001
        logger.warning("cdn_purge_failed", url=cdn_url, error=str(exc))
