...
Vector similarity alone can miss structured signals (occasion, season, user
feedback). These helpers adjust scores using domain metadata, then re-sort.
"""
...
def rerank_fashion_documents(
    ...
    *,
    ...
    """Boost fashion KB docs whose metadata aligns with the query context."""
    if not docs:
        return []
...
def rerank_outfit_history(
    ...
    *,
    ...
    """Boost past outfits the user saved, wore, or rated highly."""
    if not records:
        return []
...
def rerank_packing_memories(
    ...
    *,
    ...
    """Boost packing memories with matching purpose or destination."""
    if not records:
        return []
    ...
    for rec in records:
        ...
        missing = rec.get("missing_items") or []
        if missing:
            score += 0.02
