...
class FirestoreFollowService:
    ...
    async def follow(self, follower_id: UUID, following_id: UUID) -> None:
        ...
        await db.collection(_FOLLOWS).document(doc_id).set({
    ...
    async def follower_count(self, user_id: UUID) -> int:
        db = get_db()
        query = db.collection(_FOLLOWS).where(
            filter=FieldFilter("following_id", "==", str(user_id))
        )
        count = 0
    ...
    async def following_count(self, user_id: UUID) -> int:
        db = get_db()
        query = db.collection(_FOLLOWS).where(
            filter=FieldFilter("follower_id", "==", str(user_id))
        )
        count = 0
        async for _ in query.stream():
            count += 1
...
class FirestoreGroupService:
    ...
    async def get_by_invite_code(self, invite_code: str) -> dict[str, Any] | None:
        db = get_db()
        query = db.collection(_GROUPS).where(
    ...
    async def get_user_groups(self, user_id: UUID) -> list[dict[str, Any]]:
        ...
        db = get_db()
        member_query = db.collection(_GROUP_MEMBERS).where(
    ...
    async def get_public_groups(self, exclude_user_id: UUID) -> list[dict[str, Any]]:
        ...
        db = get_db()
        query = db.collection(_GROUPS).where(
            filter=FieldFilter("is_private", "==", False)
        ).limit(50)
    ...
    async def delete_group(self, group_id: UUID) -> None:
        ...
        member_query = db.collection(_GROUP_MEMBERS).where(
            filter=FieldFilter("group_id", "==", str(group_id))
        )
        async for doc in member_query.stream():
...
class FirestoreGroupMemberService:
    ...
    async def add_member(
        self, group_id: UUID, user_id: UUID, role: str = "member"
    ) -> dict[str, Any]:
        db = get_db()
    ...
    async def get_membership(
        ...
        db = get_db()
        doc = await db.collection(_GROUP_MEMBERS).document(
            _member_doc_id(group_id, user_id)
        ).get()
    ...
    async def remove_member(self, group_id: UUID, user_id: UUID) -> None:
        db = get_db()
        await db.collection(_GROUP_MEMBERS).document(
            _member_doc_id(group_id, user_id)
        ).delete()
    ...
    async def get_members(self, group_id: UUID) -> list[dict[str, Any]]:
        ...
        db = get_db()
        query = db.collection(_GROUP_MEMBERS).where(
            filter=FieldFilter("group_id", "==", str(group_id))
        )
        members = []
        async for doc in query.stream():
