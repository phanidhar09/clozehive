...
def _mock_result(closet_items: list[dict[str, Any]], occasion: str, *, degraded: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        # When true, this is a rules-based fallback because the AI stylist was
        # unavailable — the UI can show a "smart picks (stylist offline)" badge.
        "degraded": degraded,
    }
...
Silhouette: use each wardrobe item's "fit" field (slim/regular/relaxed/oversized/tailored) to balance proportions — pair a relaxed/oversized piece with a slim/fitted one; avoid volume-on-volume. When fit is missing, judge proportion from the item name/category.{style_block}{rag_block}"""
    ...
    # AI failed or returned unusable output — degrade to rules-based picks.
    return _mock_result(closet_items, occasion, degraded=True)
