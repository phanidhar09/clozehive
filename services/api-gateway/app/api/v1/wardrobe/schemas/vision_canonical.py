...
Maps alternate keys (garment_type, color_primary, …) into one predictable
schema for API responses and closet row creation.
"""
...
from app.api.v1.wardrobe.schemas.closet import _coerce_str_list, coerce_closet_category
...
def normalize_vision_ai_dict(raw: dict[str, Any]) -> dict[str, Any]:
    """
    Merge legacy / alternate LLM keys into canonical keys before Pydantic.

    Output keys match :class:`NormalizedVisionAIOutput` field names.
    """
...
class NormalizedVisionAIOutput(BaseModel):
    """
    Validated, normalized single-item vision JSON (OpenAI or similar).

    Built from ``normalize_vision_ai_dict``; safe defaults on partial input.
    """
...
def parse_vision_ai_payload(
    raw: Any, *, source: Literal["openai", "bulk", "fallback"] = "openai"
) -> NormalizedVisionAIOutput:
    """
    Validate arbitrary JSON from an LLM into :class:`NormalizedVisionAIOutput`.

    Malformed or non-dict input becomes safe defaults (clamped category, empty lists).
    """
    if not isinstance(raw, dict):
        raw = {}
    try:
...
def normalized_to_legacy_upload_dict(n: NormalizedVisionAIOutput) -> dict[str, Any]:
    """Shape compatible with ``closet._item_from_vision`` and older clients."""
    return {
        "name": n.name,
        "category": n.category,
        "color": n.color,
...
def normalized_to_bulk_api_dict(
    ...
    *,
    ...
    """
    Dict shape expected by bulk ingest, crop enrichment, and legacy bulk consumers.

    Preserves ``secondary_colors``, ``fit``, ``warnings`` from ``raw`` when present.
    """
...
def vision_analysis_item_to_normalized(v: Any) -> NormalizedVisionAIOutput:
    """Map a vision pipeline :class:`~app.schemas.closet.VisionAnalysisItem` to normalized core."""
    if v is None:
        return parse_vision_ai_payload({}, source="fallback")
    payload = {
        "name": getattr(v, "name", None),
        "category": getattr(v, "category", None),
        "subcategory": getattr(v, "subcategory", None),
        "primary_color": getattr(v, "primary_color", None),
        "brand": getattr(v, "brand", None),
        "material": getattr(v, "material", None),
        "pattern": getattr(v, "pattern", None),
        "season": getattr(v, "season", None),
        "occasions": getattr(v, "occasions", None),
        "description": getattr(v, "description", None),
        "confidence_score": getattr(v, "confidence_score", None),
        "style_tags": getattr(v, "style_tags", None),
    }
    return parse_vision_ai_payload(payload, source="openai")
