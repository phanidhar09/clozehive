...
def _request_fingerprint(request: Request) -> str:
    """Stable 16-char hex fingerprint of the request's IP + User-Agent.

    Used to bind the OAuth state token to the browser that initiated the login
    so an attacker cannot replay a stolen callback URL against a different session
    (CSRF on the OAuth flow).

    We use X-Forwarded-For first (Render / any reverse-proxy sets it) and fall
...
async def _cleanup_user_uploads(image_urls: list[str], user_id: str) -> None:
    ...
    account deletion.
    """
    from app.core.upload_service import delete_upload
...
async def delete_account(
    ...
    # Transactional outbox: record the cross-service purge intent in the SAME
    # transaction as the deletion so it can never be lost, even if the inline
    # attempt below fails. A reconciliation loop retries pending rows.
    await account_purge_service.record_purge(session, uid)
    ...
    # Inline best-effort purge of closet-service data; on failure the outbox row
    # stays pending and the reconciliation loop keeps retrying until it succeeds.
    bg.add_task(account_purge_service.run_purge_for_user, user_id)
                ...
                "messages": [
                    {
                        "id": _str(m.id),
                        "role": _str(m.role),
                        # ORM column is `message`; expose it under the portable
                        # `content` key. Reading "content" via getattr returned
                        # None for every row.
...
async def google_login(request: Request):
    ...
    stored = await cache_service.state_set(_OAUTH_STATE_PREFIX + state, {"v": 1, "fp": fp}, _OAUTH_STATE_TTL)
    if not stored:
        logger.error(
...
async def google_callback(
    ...
    # Validate CSRF state stored in the state Redis — also verifies the request
    # fingerprint so a stolen callback URL cannot be replayed from a different
    # browser/IP. Use state_getdel so the token is atomically consumed: it lives
    # on the state Redis (state_set), so it must be read+deleted there, not via
    # cache_service.delete (which targets the evictable cache Redis and is a no-op
    # here). Atomic get-and-delete also closes the replay window entirely.
    stored = await cache_service.state_getdel(_OAUTH_STATE_PREFIX + state)
    if not stored:
    ...
    stored_fp = stored.get("fp", "") if isinstance(stored, dict) else ""
    incoming_fp = _request_fingerprint(request)
    # Always compare — an empty/absent stored fingerprint means tampering, not a
    # pass. google_login always stores a non-empty fp, so a blank one is rejected.
    if stored_fp != incoming_fp:
        # Fingerprint mismatch — callback came from a different browser/IP than
        # the one that initiated the login. Reject and force a fresh login.
        logger.warning(
            "google_oauth_csrf_fingerprint_mismatch",
            state_prefix=state[:8],
            hint="callback IP/UA differs from login — possible CSRF attempt",
        )
        return RedirectResponse(
            url=_oauth_login_redirect(frontend, error="oauth_invalid_state"),
            status_code=302,
    ...
    try:
        ...
        auth_result, refresh_raw = await svc.google_oauth_login(
            google_id, email, name, avatar_url, email_verified=email_verified
        )
...
async def oauth_exchange(response: Response, body: _OAuthExchangeRequest):
    ...
    # Atomic GET-and-DELETE: a single Redis round-trip ensures the code can never
    # be read by two concurrent requests — the value disappears the moment the
    # first caller retrieves it, so replay attacks are impossible even under load.
    stored = await cache_service.state_getdel(redis_key)
    if not stored:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
