...
from app.api.v1.intelligence.services.agents import travel_advisor
...
# ── Packing advisor (tool-calling agent) ──────────────────────────────────────
...
@router.post("/{trip_id}/ask", status_code=status.HTTP_200_OK)
# Tighter than the CRUD routes: one call runs a tool-calling loop of up to
# (agent_max_iterations + 1) generations plus cached web lookups.
@limiter.limit("6/minute")
async def ask_travel_advisor(
    request: Request,
    trip_id: UUID,
    body: TravelAdvisorRequest,
    user_id: CurrentUser,
    session: AsyncSession = Depends(get_session),
):
    """Answer a follow-up question about a trip's generated packing plan.

    The plan itself is generated by a grounded pipeline and stays authoritative —
    this only explains and extends it ("will I be warm enough?", "dress code for
    the temple?"). Gated behind ``TRAVEL_ADVISOR_AGENT_ENABLED``.
    """
    if not settings.travel_advisor_agent_enabled:
        raise ServiceUnavailableError("The travel advisor is not enabled in this environment.")

    return await travel_advisor.advise(session, user_id, str(trip_id), body.question)
