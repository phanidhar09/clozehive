    ...
    day_plans = [
        {
            "outfits": [
                {
                    "items": [
                        {"closet_item_id": "real-1", "item_name": "Blue Oxford Shirt",
                         "category": "tops", "source": "from_closet"},
                        {"closet_item_id": "ghost-9", "item_name": "Designer Trench Coat",
                         "category": "outerwear", "source": "from_closet"},
                    ]
                }
            ]
        }
...
def test_grounding_repairs_wrong_id_via_name_match():
    ...
    day_plans = [
        {"outfits": [{"items": [
            {"closet_item_id": "", "item_name": "Black Ankle Boots",
             "category": "shoes", "source": "from_closet"},
        ]}]}
    ]
    grounded, corrected = ps._ground_day_plans_to_closet(day_plans, closet)
...
def test_grounding_leaves_missing_recommended_alone():
    ...
    day_plans = [
        {"outfits": [{"items": [
            {"closet_item_id": None, "item_name": "Buy: Swim Trunks",
             "category": "bottoms", "source": "missing_recommended"},
        ]}]}
    ]
    grounded, corrected = ps._ground_day_plans_to_closet(day_plans, closet)
    assert corrected == 0
...
def test_weather_fit_score_meets_threshold():
    ...
    for sc in _SCENARIOS:
        ...
        ranked = ps._rank_closet_for_trip(
            closet, weather, [], sc["purpose"], None, ps.BAG_CONSTRAINTS["carry_on"]
        )
...
def test_grounding_rate_is_total():
    ...
    day_plans = [
        {"outfits": [{"items": [
            ...
            {"closet_item_id": "zzz", "item_name": "Fake Blazer", "category": "outerwear", "source": "from_closet"},
            {"closet_item_id": "", "item_name": "Real Jeans", "category": "bottoms", "source": "from_closet"},
        ]}]}
    ]
    grounded, _ = ps._ground_day_plans_to_closet(day_plans, closet)
    leaked = [
        it for it in grounded[0]["outfits"][0]["items"]
