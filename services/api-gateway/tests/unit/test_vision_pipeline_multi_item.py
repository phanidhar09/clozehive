...
from app.schemas.closet import VisionAnalysisItem, VisionAnalyzeResponse
...
async def _run_pipeline(raw_items: list[dict], image_bytes: bytes) -> VisionAnalyzeResponse:
    ...
    with (
        patch("app.services.vision_pipeline_service._cache_get", AsyncMock(return_value=None)),
        patch("app.services.vision_pipeline_service._cache_set", AsyncMock()),
        patch(
            "app.services.vision_pipeline_service._run_detection",
            AsyncMock(return_value=_fake_detection_result(raw_items)),
        ),
        patch(
            "app.services.vision_pipeline_service._remove_bg_timed",
            AsyncMock(return_value=(image_bytes, "success_rembg")),
        ),
        patch(
            "app.services.vision_pipeline_service.enrich_detection_with_crop_analysis",
...
class TestMultiItemPipeline:
    ...
    async def test_each_item_has_unique_image_base64(self):
        ...
        with (
            ...
            patch("app.services.vision_pipeline_service._remove_bg_timed", fake_bg),
            patch(
                "app.services.vision_pipeline_service.enrich_detection_with_crop_analysis",
...
class TestCacheHitRecrop:
    ...
    async def test_cache_hit_triggers_recrop_not_original_url(self):
        ...
        from app.services.closet_preview_service import analyze_preview
        ...
        with (
            patch("app.services.closet_preview_service.run_pipeline", AsyncMock(return_value=cached_pipeline)),
            patch("app.services.closet_preview_service.persist_upload", side_effect=fake_persist),
            patch("app.services.cache_service.set", AsyncMock(return_value=True)),
        ):
