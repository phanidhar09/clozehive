...
Mirror of api-gateway's app/api/v1/wardrobe/planner.py (closet-service is the
post-cutover owner of the wardrobe domain).
"""
...
from app.services import weather_service, weekly_planner_service
...
async def _hydrated_week(
    session: DbSession, uid: UUID, start: date, end: date
) -> list[dict]:
    rows = (
        (
    ...
    if all_ids:
        item_rows = await session.execute(
            select(ClosetItem).where(
                ClosetItem.user_id == uid, ClosetItem.id.in_(all_ids)
            )
        )
...
async def _resolve_forecast_days(
    body: GeneratePlanRequest, session: DbSession, user_id: str
) -> list[dict]:
    if body.days:
        return [
            {
    ...
    if location:
        try:
            return await weather_service.fetch_weather_async(
                location, start.isoformat(), end.isoformat()
            )
        except Exception as exc:
            logger.warning(
                "planner_weather_lookup_failed", error=str(exc), location=location
            )
...
async def get_week(user_id: CurrentUser, session: DbSession, start: date | None = None):
    ...
    days = await _hydrated_week(
        session, UUID(user_id), first, first + timedelta(days=_MAX_PLAN_DAYS - 1)
    )
...
async def generate_week(
    ...
    existing_rows = (
        (
            await session.execute(
                select(PlannedOutfit).where(
                    PlannedOutfit.user_id == uid,
                    PlannedOutfit.plan_date.in_(plan_dates),
                )
            )
        )
        .scalars()
    ...
    recently_worn = [
        ...
        if i.get("last_worn") and date.fromisoformat(i["last_worn"]) >= cutoff
    ]
    ...
    plan = await weekly_planner_service.generate_week_plan(
        ...
        recently_worn_names=recently_worn,
    )
...
async def set_day(
    plan_date: date, body: UpdateDayRequest, user_id: CurrentUser, session: DbSession
):
    ...
    row = (
        await session.execute(
            select(PlannedOutfit).where(
                PlannedOutfit.user_id == uid, PlannedOutfit.plan_date == plan_date
            )
        )
    ).scalar_one_or_none()
    if row is None:
...
async def clear_day(plan_date: date, user_id: CurrentUser, session: DbSession):
    row = (
        await session.execute(
            select(PlannedOutfit).where(
                PlannedOutfit.user_id == UUID(user_id),
                PlannedOutfit.plan_date == plan_date,
            )
        )
    ).scalar_one_or_none()
    if row is None:
...
async def mark_worn(
    ...
    items = (
        (
            await session.execute(
                select(ClosetItem).where(
                    ClosetItem.user_id == uid, ClosetItem.id.in_(item_uuids)
                )
            )
        )
        .scalars()
