...
Mirror of api-gateway's app/api/v1/wardrobe/services/weekly_planner_service.py
(closet-service is the post-cutover owner of the wardrobe domain).
"""
...
from app.services.outfit_ai_service import _item_for_ai
        ...
        plan.append(
            {
                ...
                or infer_occasion(date.fromisoformat(str(day["date"]))),
    ...
    for day in days:
        ...
        if key in by_date:
            ...
            if not entry["occasion"]:
                entry["occasion"] = day.get("occasion") or infer_occasion(
...
async def generate_week_plan(
            ...
            "days": [
                {
                    ...
                    or infer_occasion(date.fromisoformat(str(d["date"]))),
                }
                for d in days
            ],
    ...
    try:
        raw = await ai_service.chat(
            [{"role": "user", "content": payload}], _PLAN_SYSTEM_PROMPT
        )
        plan = _validate_plan(parse_llm_json(raw), closet_items, days)
        if plan:
            return plan
