...
def _embedding_client() -> AsyncOpenAI:
    ...
    if _emb_client is None:
        _emb_client = wrap_openai_client(
            make_openai_client(
                settings.openai_api_key, base_url=settings.openai_api_base_url
            )
        )
    return _emb_client
...
async def generate_item_embedding(item: ClosetItem) -> list[float]:
    ...
    if not settings.openai_api_key:
        logger.warning(
            "embedding_fallback", reason="OPENAI_API_KEY not set", item_id=str(item.id)
        )
        return [0.0] * 1536
    ...
    if len(vector) != 1536:
        raise ValueError(
            f"Embedding model returned {len(vector)} dimensions; expected 1536"
        )
    return [float(v) for v in vector]


async def find_similar_items(
    session: AsyncSession, item_id: str, user_id: str, limit: int = 5
) -> list[ClosetItem]:
...
async def update_item_embedding_job(item_id: str) -> None:
    ...
    The retry below only papers over the durable-queue variant of that race,
    where a worker may pick the job up moments before the commit lands.
    """
